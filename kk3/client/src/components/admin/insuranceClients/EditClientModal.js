...
import { updateInsuranceClient } from "./FetchApi";

const EditClientModal = () => {
  const { data, dispatch } = useContext(InsuranceClientContext);
  const client = data.editClientModal.client;
  ...
  useEffect(() => {
    if (client) {
      // Check if the insurance type is a custom one (not in predefined list)
      const predefinedTypes = ["Health", "Life", "Auto", "Home", "Travel", "Business"];
      const isCustomType = !predefinedTypes.includes(client.insuranceType);

      setFormData({
        policyNumber: client.policyNumber || "",
        insuranceType: isCustomType ? "Other" : (client.insuranceType || "Health"),
        customInsuranceType: isCustomType ? client.insuranceType : "",
        policyDate: client.policyDate ? new Date(client.policyDate).toISOString().split('T')[0] : "",
        ...
        duration: client.duration || "Annual",
        status: client.status || "Active",
        expiryDate: client.expiryDate ? new Date(client.expiryDate).toISOString().split('T')[0] : "",
        notes: client.notes || "",
        success: false,
        error: false,
      });
  ...
  const handleSubmit = async (e) => {
    ...
    const responseData = await updateInsuranceClient(client._id, submitData);

    if (responseData && responseData.success) {
      setFormData({ ...formData, success: responseData.message, error: false });

      // Update client in the list
      dispatch({ type: "updateClient", payload: responseData.client });

      // Close modal after 2 seconds
      setTimeout(() => {
        dispatch({ type: "editClientModal", payload: { show: false, client: null } });
        setFormData({ ...formData, success: false });
      }, 2000);
    } else {
      setFormData({ ...formData, error: responseData.error || "Failed to update client", success: false });
    }
  ...
  if (!data.editClientModal.show || !client) return null;
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-screen overflow-y-auto">
          <div className="flex justify-between items-center p-6 border-b">
            <h2 className="text-xl font-semibold text-gray-800">Edit Insurance Client</h2>
            <button
              onClick={closeModal}
