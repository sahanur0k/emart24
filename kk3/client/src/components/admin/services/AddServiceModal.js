...
import { createService, getAllServices } from "./FetchApi";
...
const AddServiceDetail = () => {
  ...
  const submitForm = async (e) => {
    ...
    if (!fData.images || fData.images.length < 1 || fData.images.length > 2) {
      setFdata({ ...fData, error: "Please select 1 or 2 images", success: false });
      return;
    }
    ...
    let responseData = await createService(formData);
    if (responseData && responseData.success) {
      ...
      setTimeout(() => {
        dispatch({ type: "addServiceModal", payload: false });
      }, 2000);
    ...
    <Fragment>
      <div className="bg-white rounded-lg shadow-lg p-8 w-full max-w-md">
        <div className="flex items-center justify-between">
          <div className="text-2xl text-gray-700">Add Service</div>
          <div
            onClick={(e) =>
              dispatch({ type: "addServiceModal", payload: false })
            }
        ...
        <form className="my-4 space-y-4" onSubmit={submitForm}>
          ...
          <div className="flex flex-col space-y-1">
            ...
            <input
              ...
              className="px-4 py-2 border focus:outline-none"
            />
            <small className="text-gray-500">Select 1 or 2 images for your service</small>
          </div>
