...
import { adminSignupReq } from "./fetchApi";
...
const AdminSignup = (props) => {
  ...
  const formSubmit = async () => {
    ...
    try {
      let responseData = await adminSignupReq({
        name: data.name,
        email: data.email,
  ...
  return (
    <Fragment>
      <div className="text-center text-2xl mb-6">Admin Signup</div>
      <form className="space-y-4">
        <div className="flex flex-col">
          ...
          <input
            ...
            } rounded focus:outline-none focus:border-red-500`}
          />
        ...
        <div
          ...
          className="w-full text-center cursor-pointer px-3 py-2 text-white bg-red-500 hover:bg-red-600 rounded font-medium"
        >
          {data.loading ? "..." : "Create Admin Account"}
        </div>
        {data.success && (
