...
import { totalCost, quantity } from "./Mixins";
...
export const subTotal = (id) => {
  ...
  carts.forEach((item) => {
    if (item.id === id) {
      const qty = Number(item.quantitiy) || 0;
      const prc = Number(item.price) || 0;
      subTotalCost = qty * prc;
    }
  });
  return subTotalCost;
};

export const addToCart = (id, quantitiy, price) => {
  let cart = localStorage.getItem("cart")
    ? JSON.parse(localStorage.getItem("cart"))
    : [];
  let isObj = false;
