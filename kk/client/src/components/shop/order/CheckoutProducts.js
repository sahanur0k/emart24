...
export const CheckoutComponent = (props) => {
  ...
  useEffect(() => {
    ...
    cartListProduct().then((responseData) => {
  ...
  const placeOrder = async () => {
    ...
    try {
      const orderData = {
        ...
        amount: totalCost(),
        transactionId: "COD", // Mark as Cash on Delivery
      ...
      if (response && response.success) {
        ...
        setState({ ...state, loading: false, error: response.error || "Order failed" });
      }
    } catch (error) {
      setState({ ...state, loading: false, error: "Order failed. Try again." });
    }
  };
            ...
            return (
              ...
              >
                <div className="md:flex md:items-center md:space-x-4">
                  ...
                  {product.pSuperCoinReward > 0 && (
                    <div className="text-yellow-600 font-semibold text-sm mt-1">
                      Earn {product.pSuperCoinReward} Super Coins!
                    </div>
                  )}
                </div>
              </div>
